...
Shader "Space/SpaceFlowFog"
{
    Properties
    {
        ...
        _FlowPeriod ("Flow Period", float) = 0.05
    }
    ...
    SubShader
    {
        ...
        void surf (Input IN, inout SurfaceOutput o)
        {
            ...
            float phase0 = frac(_Time[1] * _FlowPeriod*0.25 + 0.5f);
            float phase1 = frac(_Time[1] * _FlowPeriod*0.25 + 1.0f);
 
            half4 tex0 = tex2D(_MainTex, IN.uv_MainTex + flowDir.xy * phase0);
            half4 tex1 = tex2D(_MainTex, IN.uv_MainTex + flowDir.xy * phase1);
 
            float flowLerp = abs((0.5f - phase0) / 0.5f);
            half4 finalColor = lerp(tex0, tex1, flowLerp);
 
            fixed a = tex2D(_MainTex, IN.uv_MainTex).a;

            fixed4 c = pow(float4(finalColor.rgb, finalColor.a * a) * IN.color, (_HighlightColor.r * _HighlightColor.g));
            c = c * _HighlightColor.a;
			/*
            fixed oldAlpha = c.a;
